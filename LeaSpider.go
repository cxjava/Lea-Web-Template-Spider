...
func NewLeaSpider() *LeaSpider {
	...
	lea.ch = make(chan bool, conf.GoroutineNum) // 仅limit个goroutine
	lea.hadDoneUrl = make(map[string]bool, conf.GoroutineNum)
	lea.exceptionUrl = make(map[string]bool, conf.GoroutineNum)
